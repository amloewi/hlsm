...
plot.lsm <- function(y, z, add=F, col="black", xlim=c(-2,2), ylim=c(-2,2)){    
...
plot.positions <- function(z, b, y, xlim=NULL, ylim=NULL){
    ...
    for(i in 1:dim(z)[3]){
        ...
        plot.lsm(y[,,i], z[ , ,i], add=T, col=i+1)
    }
}
...
plot.positions(ovals.init$z, ovals.init$b, two.ovals)
